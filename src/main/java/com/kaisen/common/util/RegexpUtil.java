...
public class RegexpUtil {
	...
	static {
		DATETIME_REGEXP_LIB
				.add("([1|2]\\d{3})/([0|1]?\\d{1})/([0|1|2|3]?\\d{1})");
		DATETIME_REGEXP_LIB
				.add("([1|2]\\d{3})\\-([0|1]?\\d{1})\\-([0|1|2|3]?\\d{1})");
	}
	...
	public static String[] dateMatch(String text) {
		...
		for (int i = 0; i < DATE_REGEXP_LIB.length; i++) {
			String[][] _date = RegexpUtil.matchMultiGroups(text,
					DATE_REGEXP_LIB[i]);
		...
		for (int i = 0; i < tmp.length; i++) {
			String t = tmp[i];
			tmp[i] = t.substring(0, 4) + "-" + t.substring(4, 6) + "-"
					+ t.substring(6, 8);
		}
	...
	/**
	 * <font color='red'>匹配正则第一个有效括号内第一个结果。</font> <br/>
	 * <font color=
	 * 'green'>该四组matchGroup,matchGroups,matchMultiGroup,matchMultiGroups方法体是有区别的，请务必认真看手册</font
	 * >
	 * <p>
	 * 举例:
	 * </p>
	...
	/**
	 * <font color='red'>匹配正则第一个有效括号内所有结果，结果集是一维数组形式。</font> <br/>
	 * <font color=
	 * 'green'>该四组matchGroup,matchGroups,matchMultiGroup,matchMultiGroups方法体是有区别的，请务必认真看手册</font
	 * >
	 * <p>
	...
	/**
	 * <font color='red'>匹配正则所有有效括号内第一组结果，结果集是一维数组形式，分别对应每个有效括号匹配到的结果集。</font> <br/>
	 * <font color=
	 * 'green'>该四组matchGroup,matchGroups,matchMultiGroup,matchMultiGroups方法体是有区别的，请务必认真看手册</font
	 * >
	 * <p>
	...
	/**
	 * <font color=
	 * 'red'>匹配正则所有有效括号内所有结果，结果集是二维数组形式，一维表式结果集总数，二维对应每个有效括号匹配到的结果集。</font> <br/>
	 * <font color=
	 * 'green'>该四组matchGroup,matchGroups,matchMultiGroup,matchMultiGroups方法体是有区别的，请务必认真看手册</font
	 * >
	 * <p>
	 ...
	 */
	/*
	 * public static String domainNameMatch(String url) { for(int
	 * i=0;i<URL_REGEXP_LIB.size();i++) { String regexp =
	 * (String)URL_REGEXP_LIB.get(i); Matcher m = matcher(url,regexp);
