...
public class CodeUtil {
	...
	public static byte[] BASE64Decoder(String base64) {
		try {
			return (null == base64) ? null : new Base64().decode(base64);
		} catch (Exception e) {
	...
	public static String BASE64Encoder(String str) {
		return (null == str) ? null : (new Base64().encodeToString(str
				.getBytes()));
	}
	...
	public static int md5string2int(String src) {
		return (src == null) ? null : new BigInteger(1, md5(src.getBytes()))
				.intValue();
	}
		...
		for (int i = 0; i < charArray.length; i++) {
			...
			if ((i + 1) < charArray.length && '&' == charArray[i]
				...
				for (int j = i + 2; j < charArray.length; j++) {
					...
					// if end of number with ';',do convert to non_ascii char
					// and append.
					else if (';' == charArray[j]) {
						// do like '&#number;'
						if (number.length() != 0) {
							...
							try {
								...
								non_ascii = (char) (Integer.valueOf(number
										.toString()).intValue());
								buf.append(non_ascii);
							} catch (Exception e) {
								// out of convert char integer.
								// EXP:'&#1111111111;'.
	...
	public static String URLDecoder(String s) {
		try {
			...
			logger.error("URLDecoder(" + s + ") error!", e);
			return s;
		}
	}
	...
	public static String URLDecoder(String s, String charset) {
		try {
			...
			logger.error("URLDecoder(" + s + "," + charset + ") error!", e);
			return s;
		}
	}
	...
	public static String URLEncoder(String s) {
		try {
			...
			logger.error("URLEncoder(" + s + ") error!", e);
			return s;
		}
	}
	...
	public static String URLEncoder(String s, String charset) {
		try {
			...
			logger.error("URLEncoder(" + s + "," + charset + ") error!", e);
			return s;
		}
	}
